  ...
  public Collection<String> getAllowedValues() {
    ...
    Core.getInstance().getAllCompanies().values().forEach(company -> {
      if(company.getOptionChain().getPuts() != null){
        company.getOptionChain().getPuts().forEach(optionLink -> values.add(optionLink.getContractSize()));
      }
      if(company.getOptionChain().getCalls() != null){
        company.getOptionChain().getCalls().forEach(optionLink -> values.add(optionLink.getContractSize()));
      }
    });
